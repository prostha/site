...
const data = fetch("https://api.github.com/repos/prostha/runtime/releases", {
	...
	response.ok ? (response.json() as Promise<Release[]>) : [],
);
					...
					if (
						...
						if (names.length > 0) {
							return names
								.map(
									(name) =>
										`[![${name}](https://github.com/${name}.png?size=48)](https://github.com/${name})`,
								)
								.join("");
						}
					}
...
export const metadata: Metadata = createMetadata({
	title: "Changelog",
	description: "Latest changes, fixes, and updates.",
});
