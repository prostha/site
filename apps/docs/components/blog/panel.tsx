					...
					{post ? (
						<>
							...
							{post.toc && post.toc.length > 0 && (
								<div className="hidden lg:block border-t border-foreground/6 pt-4 mt-4">
									<p className="text-xs font-mono text-foreground/40 uppercase tracking-wider mb-2">
										On this page
									</p>
									<nav className="space-y-1.5 max-h-[35vh] overflow-y-auto subtle-scrollbar">
										{post.toc.map((item, index) => (
											<a
												key={index}
												href={item.url}
												style={{ paddingLeft: `${(item.depth - 2) * 0.75}rem` }}
												className="block text-xs text-foreground/60 hover:text-foreground/90 transition-colors truncate"
											>
												{item.title}
											</a>
										))}
