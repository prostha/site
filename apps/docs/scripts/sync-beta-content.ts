...
if (process.env.BETA_DOCS_SKIP === "1") {
	console.log(
		"Beta documentation synchronization skipped via environment configuration.",
	);
	process.exit(0);
}
...
console.log(
	`Initiating synchronization of beta documentation from remote branch "${process.env.BETA_DOCS_BRANCH ?? "main"}" to destination "content/docs-beta".`,
);
...
try {
	...
	if (!existsSync(source)) {
		throw new Error(
			`Verification failed: Source path not found after sparse checkout at "${source}".`,
		);
	}
